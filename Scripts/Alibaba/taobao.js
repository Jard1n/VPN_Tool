// 地址：https://kelee.one/Resource/JavaScript/Taobao/Taobao_remove_ads.js
/*
      ...
      if (obj?.data?.containers?.splash_home_base) {
        ...
        if (splash?.base?.sections?.length > 0) {
          for (let items of splash.base.sections) {
            // 修复：添加检查，确保 items.bizData 存在且是对象
            if (items?.bizData && typeof items.bizData === 'object' && "taobao-splash" in items.bizData) {
              // 如果 items.bizData["taobao-splash"]?.data 存在且是数组，继续处理
              if (items?.bizData?.["taobao-splash"]?.data?.length > 0) {
                for (let item of items.bizData["taobao-splash"].data) {
                  ...
                  item.waitTime = "0";  // 注意：这里有重复赋值，原代码如此，保留
